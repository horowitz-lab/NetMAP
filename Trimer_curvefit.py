...
A_squared = c1(freq, 5, 5, 3, 5, .1, .1, .1, 1, 5, 2, 1)
...
popt, pcov = curve_fit(curve_func, freq, A_squared, p0=initial_guess)
...
plt.scatter(freq, A_squared, label='Original Data')
plt.xlabel('Frequency (f)')
plt.ylabel('A_squared')
plt.title('Curve Fitting with Three Peaks')
plt.grid(True)

# Generate points for the fitted curve
freq_fit = np.linspace(min(freq), max(freq), 500)
A_squared_fit = curve_func(freq_fit, *popt)

# Plot the fitted curve
plt.plot(freq_fit, A_squared_fit, color='red', label='Fitted Curve')
...
# Calculate R-squared
r_squared = r2_score(A_squared, curve_func(freq, *popt))
