...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 11 09:49:54 2024
...
import random
import pandas as pd
from Trimer_simulator import im1, im2, im3

#list to store the dataframe for each of the three masses in the trimer system
all_data = []
...
for mass in ['Mass 1', 'Mass 2', 'Mass 3']:
    ...
    #t = phase 
    def imaginary_function(w, k1, k2, k3, k4, b1, b2, b3, F, m1, m2, m3):
        if mass == 'Mass 1':
    ...
    im_part = imaginary_function(freq, 3, 3, 3, 0, 2, 2, 2, 1, 5, 5, 5)
    ...
    for trial in range(num):
        try:
            ...
            #curve fitting
            model = Model(imaginary_function)
            ...
            result = model.fit(im_part, params, w=freq)
            ...
            r_squared = 1 - result.residual.var() / np.var(im_part)
            ...
            plt.plot(freq, im_part, 'bo', label='Original Data')
            
            #generate points for fitted curve
            freq_fit = np.linspace(min(freq),max(freq), 500) #more w-values than before
            im_fit = result.model.func(freq_fit, **result.best_values)
            
            #graph fitted curve
            plt.plot(freq_fit, im_fit, '-', label='Fitted Curve')
            ...
            plt.savefig(f'/Users/Student/Desktop/Summer Research 2024/GitHub/NetMAP/Generating Random Params - Curve Fit/{mass} plots - imaginary part/plot_{trial+1}.png')
            plt.show()
...
file_path = '/Users/Student/Desktop/Summer Research 2024/GitHub/NetMAP/Generating Random Params - Curve Fit/Generating_Random_Params_Imaginary_Part.xlsx'
