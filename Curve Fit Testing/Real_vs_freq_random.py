...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 11 09:35:43 2024
...
import random
import pandas as pd
from Trimer_simulator import re1, re2, re3
...
for mass in ['Mass 1', 'Mass 2', 'Mass 3']:
    ...
    #t = phase 
    def real_function(w, k1, k2, k3, k4, b1, b2, b3, F, m1, m2, m3):
        if mass == 'Mass 1':
    ...
    real_part = real_function(freq, 3, 3, 3, 0, 2, 2, 2, 1, 5, 5, 5)
    ...
    for trial in range(num):
        try:
            ...
            result = model.fit(real_part, params, w=freq)
            ...
            r_squared = 1 - result.residual.var() / np.var(real_part)
            ...
            plt.plot(freq, real_part, 'bo', label='Original Data')
            
            #generate points for fitted curve
            freq_fit = np.linspace(min(freq),max(freq), 500) #more w-values than before
            real_part_fit = result.model.func(freq_fit, **result.best_values)
            
            #graph fitted curve
            plt.plot(freq_fit, real_part_fit, '-', label='Fitted Curve')
            ...
            plt.savefig(f'/Users/Student/Desktop/Summer Research 2024/GitHub/NetMAP/Generating Random Params - Curve Fit/{mass} plots - real part/plot_{trial+1}.png')
            plt.show()
...
file_path = '/Users/Student/Desktop/Summer Research 2024/GitHub/NetMAP/Generating Random Params - Curve Fit/Generating_Random_Params_Real_Part.xlsx'
